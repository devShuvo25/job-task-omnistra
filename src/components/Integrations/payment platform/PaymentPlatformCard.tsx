...
const PaymentPlatformCard = ({ platform }: PlatformProps) => {
  return (
    <motion.div
      ...
      className="group relative p-8 rounded-2xl border border-white/5 bg-[#0A0A0A] hover:bg-[#0F0F0F] transition-all cursor-pointer h-45 flex flex-col justify-between"
    >
      <div className="flex justify-between items-start">
        ...
        <div className="w-10 h-10 rounded-lg bg-black border border-white/10 flex items-center justify-center overflow-hidden p-2">
          {typeof platform.logo === 'string' ? (
            ...
            <div className="w-full h-full flex items-center justify-center  group-hover:grayscale-0 transition-all">
              {platform?.logo}
            </div>
          )}
        </div>
